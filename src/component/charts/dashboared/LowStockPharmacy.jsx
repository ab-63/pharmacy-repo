...
function LowStockPharmacy({ searData }) {
 
  const {pharmacyState,setPharmacyState} =  useAppContext()
  const lowstock = pharmacyState.filter((amount) => amount.quantity <= amount.lowStock);
        ...
        <table className="  w-full bg-cyan-200 rounded-xl text-left">
          <thead className="bg-cyan-300">
            <tr className="">
              ...
              <th className=" py-2 px-4 ">Quantiy</th>
...
export default LowStockPharmacy;
